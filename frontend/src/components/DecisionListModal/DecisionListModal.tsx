...
import {
  ...
  fetchDecisionDefinitions,
} from '@services/operaton-api';

interface DecisionListModalProps {
  onClose: () => void;
  onOpen: (xml: string, definition: DecisionDefinitionResponse) => void;
}

export default function DecisionListModal({ onClose, onOpen }: DecisionListModalProps) {
  const [definitions, setDefinitions] = useState<DecisionDefinitionResponse[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [actionInProgress, setActionInProgress] = useState<string | null>(null);
  const [confirmDelete, setConfirmDelete] = useState<DecisionDefinitionResponse | null>(null);

  const loadDefinitions = async () => {
    setLoading(true);
    setError(null);
    try {
      const defs = await fetchDecisionDefinitions();
  ...
  const handleOpen = async (def: DecisionDefinitionResponse) => {
    setActionInProgress(def.id);
    setError(null);
    try {
      const xml = await fetchDecisionDefinitionXml(def.id);
      onOpen(xml, def);
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
    } finally {
  ...
  const handleDelete = async (def: DecisionDefinitionResponse) => {
    setActionInProgress(def.deploymentId);
    setError(null);
    try {
      // cascade=true: delete deployment + any decision instances. DMN-only
      // deployments don't have running process instances, but cascade is
      // still required if a related BPMN process referenced this decision.
    ...
    >
      <div className="max-h-[70vh] w-[60rem] overflow-hidden rounded-cards bg-white shadow-200">
        <div className="flex items-center justify-between border-b-1 border-divider px-24 py-16">
          <h2 className="text-h4-sm font-semibold text-dark-primary">Decision definitions</h2>
        ...
        <div className="max-h-[calc(70vh-8rem)] overflow-y-auto">
          ...
          {!loading && definitions.length === 0 && (
            <div className="px-24 py-16 text-sm text-dark-secondary">
              No decision definitions found. Publish a DMN diagram first.
            </div>
          )}
                  ...
                  <tr key={def.id} className="border-b-1 border-divider hover:bg-gray-lighter">
                    ...
                    <td className="flex items-center justify-end gap-8 px-24 py-10">
                      {confirmDelete?.id === def.id ? (
                        <>
                          <span className="text-xs text-error">Delete this deployment?</span>
                          <button
                            type="button"
